...
Application::Application(string dossier){
  ...
  adr << std::fixed << dossier;
  dossierConfig=adr.str();
  adr << std::fixed << "/parametres.conf";
  fichierParametres=adr.str();
  adr.clear();
  adr.str("");
  ...
  adr.clear();
  adr.str("");
  std::cout << "dossier config: " << dossierConfig<<std::endl;
...
void Application::appPreferences(){
  ...
  if(fichier){ 
     while(getline(fichier, contenu)){
       ...
       if(pf[0]=="Application"){
			  appName=pf[1];
			  adr << std::fixed << "~/"<<appName<<"/Sessions";
			  usrSessions=adr.str();
			  adr.clear();
			  adr.str("");
       }
